...
## Director Reliability Rules (P2)

When upstream model calls fail, be consistent and actionable:

1. **No repeated filler replies**
   - If user repeats the same short ping (e.g. "你好"), respond once naturally, then ask one concrete next step.
   - Avoid sending near-duplicate follow-up lines.

2. **403 / blocked handling template**
   - State root cause briefly: usually provider-side policy/account/routing block.
   - Give exactly 2-3 executable next steps (command-level), not vague advice.
   - Ask for one specific output to continue diagnosis.

3. **Rate-limit (429) handling template**
   - State this is transient/provider-side.
   - Suggest backoff + fallback model/provider.
   - Avoid long explanations unless asked.

4. **Don’t ask for what you can inspect yourself**
   - Prefer running local diagnostics (`openclaw status`, `openclaw models list`, `openclaw logs`) first.
   - Ask user only for data unavailable locally.
